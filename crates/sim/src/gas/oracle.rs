...
pub fn get_fee_oracle<'a, P>(chain_spec: &ChainSpec, provider: P) -> Arc<dyn FeeOracle + 'a>
where
    P: EvmProvider + 'a,
{
    if !chain_spec.eip1559_enabled {
        return Arc::new(ConstantOracle::new(0));
    }

    match chain_spec.priority_fee_oracle_type {
        PriorityFeeOracleType::Provider => Arc::new(ProviderOracle::new(
            provider,
...
impl MaxOracle {
    ...
    pub(crate) fn add(&mut self, oracle: Box<dyn FeeOracle>) {

...
pub trait VerificationGasEstimator: Send + Sync {
    ...
    where
        F: Send + Sync + Fn(Self::UO, EstimateGasArgs<'_>) -> Fut,
        Fut: Send
            + Sync
            + Future<Output = Result<Result<EstimateGasResult, Bytes>, GasEstimationError>>;
}

#[derive(Debug, Clone)]
pub struct EstimateGasArgs<'a> {
    pub block_hash: B256,
    pub state_overrides: &'a StateOverride,
    pub min_gas: u128,
    pub max_gas: u128,
    pub rounding: u128,
    ...
    UO: UserOperation,
{
    ...
    where
        F: Send + Sync + Fn(UO, EstimateGasArgs<'_>) -> Fut,
        Fut: Send
            + Sync
            + Future<Output = Result<Result<EstimateGasResult, Bytes>, GasEstimationError>>,
    {
        ...
        loop {
            ...
            let call_contract_result = call_contract(
                ...
                EstimateGasArgs {
                    block_hash,
                    state_overrides: &local_state_override,
                    min_gas,
                    max_gas,
                    rounding: 4096,
                    is_continuation,

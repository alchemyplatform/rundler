...
pub(crate) fn response_from_chunks(
    ...
    match schema {
        ProtocolSchema::StatusV1 => {
            if chunks.len() != 1 {
                return invalid_response("wrong number of chunks");
            };
            match chunks.pop().unwrap().0 {
        ...
        ProtocolSchema::PooledUserOpHashesV1 => {
            ...
            match chunks.pop().unwrap().0 {
                Ok(SszChunk::PooledUserOpHashes(hashes)) => {
                    Ok(Response::PooledUserOpHashes(hashes))
                }
                Ok(_) => invalid_response("wrong kind"),
                Err(err) => Err(err)?,
            }
        ...
        ProtocolSchema::PooledUserOpsByHashV1 => {
            // NOTE: number of chunks is is checked by protocol handler
...
pub(crate) struct PooledUserOpsByHashChunkSsz {
    user_op: UserOperationSsz,
}

#[derive(Debug, Encode, Decode)]
struct UserOperationSsz {
    sender: Vec<u8>,
    nonce: U256,
    init_code: Vec<u8>,
    call_data: Vec<u8>,
...
impl ProtocolSchema {
    pub fn ssz_request_limits(&self) -> (usize, usize) {
        match self {
    ...
    pub fn ssz_response_limits(&self) -> (usize, usize) {
        match self {
            ProtocolSchema::StatusV1 => (0, MAX_SSZ_SIZE),

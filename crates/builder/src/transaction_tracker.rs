    ...
    T: TransactionSender,
{
    pub(crate) async fn new(
        ...
        let nonce_fut = provider.get_transaction_count(signer.address());
        let balance_fut = provider.get_balance(signer.address(), None);
        let (nonce, balance) = tokio::try_join!(nonce_fut, balance_fut)?;

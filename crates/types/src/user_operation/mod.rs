...
pub trait UserOperation: Debug + Clone + Send + Sync + 'static {
    ...
    fn required_pre_verification_gas(
        ...
        self.required_pre_verification_gas_with_limits(
            chain_spec,
            bundle_size,
            da_gas,
    ...
    /// Returns the required pre-verification gas for the given user operation with optional gas limits
    ///
    /// `bundle_size` is the size of the bundle
    /// `da_gas` is the DA gas cost for the user operation, calculated elsewhere
    /// `verification_gas_limit` is the verification gas limit (if available from estimation)
    ...
    fn required_pre_verification_gas_with_limits(
        &self,
        chain_spec: &ChainSpec,
        bundle_size: usize,
        da_gas: u128,
        ...
        if chain_spec.charge_gas_limit_via_pvg {
            let total_vgl =
                verification_gas_limit.unwrap_or_else(|| self.total_verification_gas_limit());

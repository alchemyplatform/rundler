...
impl UserOperationOptionalGas {
    ...
    pub fn max_fill(&self, chain_spec: &ChainSpec) -> UserOperation {
        ...
        if let Some(auth) = self.eip7702_auth_address {
            let auth = Eip7702Auth::from_chain_id_and_address(chain_spec.id, auth).max_fill();
    ...
    pub fn random_fill(&self, chain_spec: &ChainSpec) -> UserOperation {
        ...
        if let Some(auth) = self.eip7702_auth_address {
            let auth = Eip7702Auth::from_chain_id_and_address(chain_spec.id, auth).random_fill();
    ...
    pub fn into_user_operation_builder(
        ...
        let authorization_tuple = self
            ...
            .map(|address| Eip7702Auth::from_chain_id_and_address(chain_spec.id, address));

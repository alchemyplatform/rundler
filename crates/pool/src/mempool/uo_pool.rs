...
pub(crate) struct UoPool<Components: UoPoolProvidersT> {
    config: PoolConfig,
    providers: Components,
    state: RwLock<UoPoolState<Components::DaGas>>,
    paymaster: PaymasterTracker<Components::EntryPoint>,
...
impl<Providers> UoPool<Providers>
where
    Providers: UoPoolProvidersT,
{
    pub(crate) fn new(
        config: PoolConfig,
        providers: Providers,
        event_sender: broadcast::Sender<WithEntryPoint<OpPoolEvent>>,
        paymaster: PaymasterTracker<Providers::EntryPoint>,
        reputation: Arc<AddressReputation>,
    ) -> Self {
...
#[async_trait]
impl<Components> Mempool for UoPool<Components>
where
    Components: UoPoolProvidersT,
{
    async fn on_chain_update(&self, update: &ChainUpdate) {
        ...
        match self.providers.prechecker().update_fees().await {
...
pub(crate) trait UoPoolProvidersT: Send + Sync {
    ...
    type EntryPoint: EntryPoint + SimulationProvider<UO = Self::UO> + Clone;
    type DaGas: DAGasOracleSync + Clone;
    ...
    fn da_gas_oracle(&self) -> &Option<Self::DaGas>;
}

pub(crate) struct UoPoolProviders<E, P, S, EP, D> {
    pub(crate) evm: E,
    pub(crate) prechecker: P,
    pub(crate) simulator: S,
    pub(crate) entry_point: EP,
    pub(crate) da_gas_oracle: Option<D>,
}

impl<E, P, S, EP, D> UoPoolProvidersT for UoPoolProviders<E, P, S, EP, D>
where
    E: EvmProvider,
    S: Simulator,
    <S as Simulator>::UO: UserOperation + From<UserOperationVariant> + Into<UserOperationVariant>,
    P: Prechecker<UO = <S as Simulator>::UO>,
    EP: EntryPoint + SimulationProvider<UO = <S as Simulator>::UO> + Clone,
...
mod tests {
    ...
    fn create_pool(ops: Vec<OpWithErrors>) -> UoPool<impl UoPoolProvidersT> {
    ...
    async fn create_pool_with_entrypoint_insert_ops(
        ops: Vec<OpWithErrors>,
        entrypoint: MockEntryPointV0_6,
    ) -> (UoPool<impl UoPoolProvidersT>, Vec<UserOperationVariant>) {
        let uos = ops.iter().map(|op| op.op.clone()).collect::<Vec<_>>();

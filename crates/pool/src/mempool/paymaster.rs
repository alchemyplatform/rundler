...
impl PaymasterTracker {
    ...
    pub(crate) fn update_paymaster_balance_after_reorg(
        &mut self,
        paymaster: Address,
        deposit_amount: U256,
    ) {
    ...
    fn update_paymaster_balance(
        ...
        if let Some(replacement) = self.user_op_fees.get_mut(id) {
            ...
            replacement.replace_values(max_op_cost, paymaster);
...
impl UserOpFees {
    ...
    fn replace_values(&mut self, max_op_cost: U256, paymaster: Address) {

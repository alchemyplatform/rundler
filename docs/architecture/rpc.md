...
- A block range object: `{ fromBlock, toBlock }`, where each field is an optional string containing either a hex-encoded block number (`0x`-quantity, e.g. `"0x64"` — decimal JSON numbers are rejected) or a block tag (`"latest"`, `"earliest"`, `"pending"`, `"safe"`, `"finalized"`; case-insensitive). Unknown fields in the object are rejected. When the node enforces a maximum event block range (`--user_operation_event_block_distance`, or a per-request `X-Rundler-Max-Block-Range` override), a one-sided range is accepted and expanded to a maximum-width window anchored at the supplied bound: `{ fromBlock }` searches `[fromBlock, fromBlock + max]` (the upper bound is capped at the chain head), and `{ toBlock }` searches `[toBlock - max, toBlock]`. A range with both bounds wider than the maximum, with `fromBlock` greater than `toBlock`, or with a one-sided `fromBlock` past the chain head, is rejected with an invalid params error (`-32602`).

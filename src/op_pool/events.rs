...
type NewBlockCallback = dyn Fn(&NewBlockEvent) + Send + Sync + 'static;

pub struct EventListener {
    provider: Arc<Provider<Ws>>,
    entry_point: EntryPoint<Provider<Ws>>,
    subscriptions: Mutex<Vec<Box<NewBlockCallback>>>,
}
...
impl EventListener {
    ...
    pub fn subscribe(&self, callback: impl Fn(&NewBlockEvent) + Send + Sync + 'static) {

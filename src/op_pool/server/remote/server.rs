...
pub async fn spawn_remote_mempool_server<M: Mempool>(
    ...
    let handle = tokio::spawn(async move {
        ...
        tokio::try_join!(pool_handle, server_handle)?;
        Ok(())
    });
...
struct OpPoolImpl<M: Mempool> {
    ...
    block_subscriptions: Mutex<Vec<Sender<Result<NewBlock, Status>>>>,
}
    ...
    M: Mempool,
{
    pub fn new(chain_id: u64, mempools: Arc<MempoolGroup<M>>) -> Self {
        Self {
            chain_id,
            mempools,
            block_subscriptions: Mutex::new(vec![]),
        }
    }

    pub async fn run(self: Arc<Self>, shutdown_token: CancellationToken) -> anyhow::Result<()> {
        ...
        loop {
            let block = tokio::select! {
                ...
                r = rx_blocks.recv() => {
                    r.context("Failed to receive block")?
                }
            };

            let subs = self.block_subscriptions.lock().unwrap().clone();
    ...
    M: Mempool + 'static,
{
    ...
    async fn subscribe_new_blocks(
        ...
        self.block_subscriptions.lock().unwrap().push(tx);

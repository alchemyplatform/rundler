...
pub(super) async fn fund_signers(
    ...
    if !signer_manager.fund_signers(balances) {
        println!("No signers need funding");
    }

    let count = signer_manager.addresses().len();
    while signer_manager.available() < count {
        println!(
            "Waiting for {} signers to be funded",
            count - signer_manager.available()
        );

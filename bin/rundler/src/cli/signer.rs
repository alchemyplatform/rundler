...
use clap::Args;
use rundler_signer::{FundingSettings, KmsLockingSettings, SignerKeyType, SigningScheme};
...
impl SignerArgs {
    pub fn signing_scheme(&self, num_signers: usize) -> anyhow::Result<SigningScheme> {
        ...
        bail!("No signing scheme provided (unfunded). Provide either signer.private_keys or signer.aws_kms_key_ids");
    }
    ...
    fn funding_signer_scheme(&self, num_signers: usize) -> anyhow::Result<SigningScheme> {
        ...
        let key_by_funding_key = if !self.aws_kms_key_groups.is_empty() {
            ...
            let keys = SignerKeyType::Mnemonic {
            ...
            let keys = SignerKeyType::PrivateKeys {
                private_keys: self.private_keys.clone(),
            };
